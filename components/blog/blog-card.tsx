...
export function BlogCard({ post }: BlogCardProps) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[10px] border border-border bg-surface transition duration-200 hover:-translate-y-1 hover:shadow-[0_20px_45px_-28px_hsl(var(--shadow)/0.65)]">
      {post.coverImage ? (
        <Link
          href={`/blog/${post.slug}`}
          className="block overflow-hidden border-b border-border bg-muted focus-visible:rounded-sm"
        >
          <img
            src={post.coverImage}
            alt={post.title}
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        ...
        <div className="mt-auto flex items-center justify-between gap-4 border-t border-border pt-5 text-sm">
