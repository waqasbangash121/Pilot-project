...
export async function PATCH(request: Request, { params }: ToolUpdateRouteProps) {
  ...
  try {
    ...
    const item = parseManagedContentInput(await request.json(), "tool");
    const saved = await saveManagedContent(item, editor.login, slug);

    revalidateContentRoutes("tool", saved.slug, slug);
    return NextResponse.json({ slug: saved.slug });
  } catch (error) {
    const message = error instanceof Error ? error.message : "The tool could not be saved.";
