...
export default function HyperSearchFilterPage() {
  return (
    <>
      ...
      <Section spacing="lg"><Container><div className="grid gap-8 lg:grid-cols-[1fr_0.8fr]"><div><h2 className="text-3xl font-black tracking-normal">Improve Shopify Product Discovery</h2><div className="mt-8 grid gap-4 sm:grid-cols-2">{resources.map(([title, href, body]) => <Link key={title} href={href} className="rounded-[8px] border border-border bg-surface p-5 transition hover:border-primary/50" {...analyticsAttrs("search_app_resource_click", "related_resources", title, href)}><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{body}</p></Link>)}</div></div><div><h2 className="text-3xl font-black tracking-normal">Solve the Next Storefront Bottleneck</h2><div className="mt-8 grid gap-4">{otherApps.map(([title, href, body]) => <Link key={title} href={href} className="rounded-[8px] border border-border bg-surface p-5 transition hover:border-primary/50"><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{body}</p></Link>)}</div></div></div></Container></Section>
